            ...
            <div class="row" v-for="r in hall.row" :key="r">
              ...
              <div class="seats-container" :style="dynamicPadding">
                <span
                  class="single-seat"
                  :style="dynamicWidth"
                  v-for="c in hall.column"
                  :key="c"
                  @click="chooseSeat(r, c)"
                >
                  <icon-base width="28" height="28">
                    <icon-sold-seat v-if="isInSoldSeats(r, c)" />
                    <icon-chosen-seat
                      v-else-if="chosenSeats[r - 1][c - 1] === 1"
                    />
          ...
          <div class="movie-info">
            ...
            <div class="ticket-info">
              ...
              <div class="has-ticket" v-else>
                ...
                <div class="chosen-seats">
                  <span
                    class="ticket"
                    v-for="(seat, index) in chosenSeatsCompressed"
                    :key="index"
                    >{{ seat[0] }}排{{ seat[1] }}座</span
                  >
                </div>
              </div>
            ...
            <div class="confirm">
              <el-button
                type="primary"
                @click="confirm"
                :disabled="chosenSeatsCompressed.length === 0"
                >确认选座</el-button
              >
            </div>
          </div>
...
export default {
  ...
  created() {
    ...
    for (let i = 0; i < this.hall.row; i++)
      this.chosenSeats[i] = new Array(this.hall.column).fill(0);
  ...
  methods: {
    ...
    cancelSeat(r, c) {
      this.chosenSeatsCompressed = this.chosenSeatsCompressed.filter(
        item => item[0] !== r || item[1] !== c
      );
      this.totalPrice -= this.schedule.prize;
    },
    ...
    confirm() {
      ...
      order["tickets"] = this.chosenSeatsCompressed.map(ticket => {
        return [ticket[0] - 1, ticket[1] - 1];
      });
