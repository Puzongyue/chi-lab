  ...
  <div class="main">
    <div class="count-down-wrapper">
      <div class="count-down">
        <p class="time-left">
          请在<span class="time">3分钟21秒</span>内完成支付
        </p>
        <p class="tip">超时订单会自动取消，如遇支付问题，请致电影院客服：1010-5050</p>
      </div>
    </div>
    <p class="warning">
    ...
    <div class="seats">
      ...
      <el-table class="seats-table" border stripe :data="mockSeatsInfo">
        <el-table-column prop="movie" label="影片" align="center" :show-overflow-tooltip="true">
        </el-table-column>
        ...
        <el-table-column prop="seat" label="座位" align="center">
          <template slot-scope="scope">
            ...
            <span v-bind:class="{border : index !== 0}" v-for="(ticket, index) in scope.row.seat" :key="ticket.id">
              <i>{{ ticket[0] }}</i>排<i>{{ ticket[1] }}</i>座
            </span>
          </template>
        </el-table-column>
      </el-table>
    ...
    <div class="price-and-confirm">
      <div class="price">实际支付：<span>{{ totalPrice }}</span></div>
      <div class="confirm">
...
export default {
  ...
  computed: {
    totalPrice() {
      return this.mockSeatsInfo.reduce((prev, curr) => { return prev + curr.price}, 0);
    }
  },
  ...
  methods: {
    formateDate(date) {
      ...
      return `周${week[date.getDay()]} ${dateArr[0]}年${dateArr[1]}月${dateArr[2]}日 ${time.join(":")}`;
    },
